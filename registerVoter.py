...
def reg_server(root,frame1,name,sex,zone,city,passw):
    ...
    Label(frame1, text=txt,bg="steel blue" , font=('Helvetica', 18, 'bold')).grid(row = 2, column = 1, columnspan=2)
...
def Register(root,frame1):
    ...
    Label(frame1, text="Zone:           ", bg="steel blue" ,anchor="e", justify=LEFT).grid(row = 5,column = 0)
    Label(frame1, text="City:             ",bg="steel blue" , anchor="e", justify=LEFT).grid(row = 6,column = 0)
    ...
    reg = Button(frame1, text="Register",bg="light steel blue" ,command = lambda: reg_server(root, frame1, name.get(), sex.get(), zone.get(), city.get(), password.get()), width=10)
